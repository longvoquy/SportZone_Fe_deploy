...
export default function FieldCoachBookingListPage() {
    ...
    useEffect(() => {
        try {
            const stored = localStorage.getItem('ownerHiddenCoachBookingIds');
            if (stored) {
    ...
    useEffect(() => {
        try {
            localStorage.setItem('ownerHiddenCoachBookingIds', JSON.stringify(hiddenIds));
        } catch (error) {
    ...
    const fetchBookings = (isInitial = false) => {
        ...
        dispatch(
            getMyFieldsBookings({
                ...
                page: currentPage,
                limit: itemsPerPage,
                type: 'field_coach', // Hard-coded for field + coach bookings
            })
        ).then(() => {
                ...
                <Card className="bg-white shadow-md rounded-none border-0 gap-0">
                    <CardHeader className="border-b">
                        ...
                        <p className="text-gray-600 text-base mt-1.5 text-start">
                            Quản lý và theo dõi tất cả các đặt sân có huấn luyện viên của bạn.
                        </p>
                        <div className="mt-4">
                            <BookingFilters
